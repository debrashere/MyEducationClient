        ...
        return (
            <div id="toolslist" className='flex-item item-box' key={index*3}>            
                <div className='item' key={index+1}>
                    ...
                    <span className='item price'><Link to={`blog/${tool['id']}`}>View Blog</Link></span>
                    <span className='item'><Link  to={`toolseditform/${tool['id']}`} tool={params(tool['id'])}>Edit</Link></span>                       
                </div>
